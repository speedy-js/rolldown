...
static BLACKLISTED: Lazy<HashSet<&'static str>> = Lazy::new(|| BUILTINS.clone().into_iter().chain(RESERVED_WORDS.clone().into_iter()).collect());

static ILLEGAL_CHARACTERS_RE: Lazy<regex::Regex> = Lazy::new(|| regex::Regex::new(r"[^$_a-zA-Z0-9]").unwrap());
